...
import { VideoDetail } from '../../video-view/video-view.js'
import { axios } from '../../../core/axios/axios.js'

let lastCoverUrl: string
...
export default Vue.extend({
  ...
  computed: {
    ...
    videoDisplayList() {
      if (!this.search) {
        ...
        return this.videoList.filter((v: VideoDetail) => {
  ...
  async mounted() {
    ...
    this.videoList = response.data.map((v: any) => {
  ...
  methods: {
    ...
    async editVideo(video: VideoDetail) {
    ...
    async deleteVideo(video: VideoDetail) {

...
import { VideoPeek } from '../home/video-peek/video-peek.js';
...
export default Vue.extend({
  ...
  computed: {
    current() {
      if (this.id) {
        return this.videos.find((v: VideoDetail) => v.id.toString() === this.id.toString())
      }
      return this.videos[0]
    },
    otherVideos() {
      if (this.id) {
        return this.videos.filter((v: VideoDetail) => v.id.toString() !== this.id.toString())
      }
      return this.video.slice(1)
    }
  },
})
export interface VideoDetail extends VideoPeek {
  url: string
  date: Date
}